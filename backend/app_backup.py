...
# Use best_tea_disease_cnn.h5 (Keras/TensorFlow format)
...
def load_model() -> None:
    ...
    try:
        logger.info(f"Loading Keras/TensorFlow model from {MODEL_PATH}")

        # Try loading with safe_mode=False for compatibility with older model formats
        try:
            model = keras.models.load_model(MODEL_PATH, safe_mode=False)
        except TypeError:
            # If safe_mode parameter doesn't exist, load normally
...
def startup_event() -> None:
    ...
    logger.info("=" * 60)
    logger.info(f"TensorFlow/Keras backend")
